<?php
defined('BASEPATH') or exit('No direct script access allowed');

class ObatImport extends CI_Controller
{

    public function index()
    {
        $data['title'] = 'Import Excel';
        $data['obat'] = $this->db->get('obat')->result();
        $this->load->view('import/index', $data);
    }

    public function create()
    {
        $data['title'] = "Upload File Excel";
        $this->load->view('import/importobat', $data);
    }

    public function excel()
    {
        if (isset($_FILES["file"]["name"])) {
            // upload
            $file_tmp = $_FILES['file']['tmp_name'];
            $file_name = $_FILES['file']['name'];
            $file_size = $_FILES['file']['size'];
            $file_type = $_FILES['file']['type'];
            // move_uploaded_file($file_tmp,"uploads/".$file_name); // simpan filenya di folder uploads

            $object = PHPExcel_IOFactory::load($file_tmp);

            foreach ($object->getWorksheetIterator() as $worksheet) {

                $highestRow = $worksheet->getHighestRow();
                $highestColumn = $worksheet->getHighestColumn();

                for ($row = 5; $row <= $highestRow; $row++) {

                    $toko_obat = $worksheet->getCellByColumnAndRow(1, $row)->getValue();
                    $si = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
                    $nib = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
                    $tgl_daftar = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
                    $masa_toko = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
                    $alamat = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
                    $ap = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
                    $telp_ap = $worksheet->getCellByColumnAndRow(8, $row)->getValue();
                    $ps = $worksheet->getCellByColumnAndRow(9, $row)->getValue();
                    $telp_ps = $worksheet->getCellByColumnAndRow(10, $row)->getValue();
                    $keterangan = $worksheet->getCellByColumnAndRow(11, $row)->getValue();
                    $status = $worksheet->getCellByColumnAndRow(12, $row)->getValue();

                    $data[] = array(
                        'toko_obat' => $toko_obat,
                        'si' => $si,
                        'nib' => $nib,
                        'tgl_daftar' => $tgl_daftar,
                        'masa_toko' => $masa_toko,
                        'alamat' => $alamat,
                        'ap' => $ap,
                        'telp_ap' => $telp_ap,
                        'ps' => $ps,
                        'telp_ps' => $telp_ps,
                        'keterangan' => $keterangan,
                        'status' => $status,
                    );
                }
            }

            $this->db->insert_batch('obat', $data);


            $this->session->set_flashdata('success', 'Data Berhasil di-import');
            redirect('ia/obat');
        } else {
            $this->session->set_flashdata('error', 'Data Gagal di-import');
            redirect('import');
        }
    }
}

    /* End of file Import.php */
    /* Location: ./application/controllers/Import.php */